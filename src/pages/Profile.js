...
export default function Profile() {
    ...
    try {
      await API.put(
        "/user/profile",
        {
          ...
          email: form.email,
          role: form.role,
          password: form.password || undefined, // ส่งแค่ถ้ามี
  ...
  // Generic onChange for user & nested profile
  const onChange = (key, value, nested = false) => {
    if (nested) {
      setForm((prev) => ({
        ...prev,
  ...
  const firstName = (form.profile.fullName || form.username || "User").split(" ")[0];
    ...
    >
          ...
          <div className="card-body p-4 p-md-5">
            ...
            <div className="d-flex align-items-center justify-content-between flex-wrap mb-4">
              <div className="d-flex align-items-center">
                <img
                  src={form.profile.profileImageUrl || defaultAvatar}
                  alt="avatar"
                  onError={(e) => {
                ...
                <div>
                  <div className="fw-semibold">
                    {form.profile.fullName || form.username || "User"}
                  </div>
            ...
            <div className="row g-3">
              <div className="col-md-6">
                ...
                <input
                  ...
                  disabled={!editMode}
                  style={{ backgroundColor: !editMode ? "#f8f9fa" : undefined }}
                />
              </div>
              ...
              <div className="col-md-6">
                ...
                <input
                  ...
                  onChange={(e) => onChange("position", e.target.value, true)}
                  disabled={!editMode}
                  style={{ backgroundColor: !editMode ? "#f8f9fa" : undefined }}
                />
              </div>
      ...
      {showModal && (
        <div className="modal fade show d-block" tabIndex="-1" role="dialog" style={{ backgroundColor: "transparent" }}>
          <div className="modal-dialog modal-dialog-centered modal-sm" role="document" style={{ pointerEvents: "none" }}>
            <div
              className="modal-content rounded-3 shadow-sm text-center px-3 py-2 animate__animated animate__fadeInDown"
              style={{ display: "inline-block", pointerEvents: "auto", minWidth: "180px", backgroundColor: "#ffffff", fontSize: "0.9rem" }}
            >
              <button
                type="button"
                onClick={() => setShowModal(false)}
                style={{ position: "absolute", top: 4, right: 6, border: "none", background: "transparent", fontSize: "1rem", cursor: "pointer" }}
              >
                ×
              </button>
